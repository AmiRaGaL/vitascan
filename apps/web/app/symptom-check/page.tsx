...
type Step = 'body-area' | 'symptom' | 'questions' | 'health-profile' | 'results';
...
export default function SymptomChecker() {
  ...
  const handleBodyAreaSelect = async (area: BodyArea) => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`${API_URL}/symptom-sessions/symptom-categories/${area.id}`);
  ...
  const handleSymptomSelect = async (symptom: SymptomCategory) => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`${API_URL}/symptom-sessions/symptom-questions/${symptom.id}`);
      const data = await res.json();
      setQuestions(data);
      setStep('questions');
    } catch (error) {
  ...
  const resetWizard = () => {
    ...
    setAnswers({});
    setHealthProfile({ age: '', sex_at_birth: '', chronic_conditions: '', medications: '', allergies: '' });
    setResult(null);
  };
    ...
    <main className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-purple-50 p-4 md:p-8">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="text-center mb-8">
          <h1 className="text-4xl font-bold text-gray-900 mb-2">VitaScan AI Triage</h1>
          <p className="text-gray-600">Intelligent symptom assessment powered by AI</p>
        </div>
        ...
        {step !== 'results' && (
          <div className="mb-8">
            <div className="flex justify-between items-center mb-2">
              {['Body Area', 'Symptom', 'Questions', 'Health Info'].map((label, idx) => {
                const steps: Step[] = ['body-area', 'symptom', 'questions', 'health-profile'];
                ...
                return (
                  <div key={label} className="flex items-center flex-1">
                    ...
                    <span className={`ml-2 text-sm hidden md:inline ${isActive ? 'font-semibold text-blue-600' : 'text-gray-500'}`}>
                      {label}
                    </span>
                    {idx < 3 && <div className={`flex-1 h-1 mx-2 ${isComplete ? 'bg-green-500' : 'bg-gray-200'}`} />}
                  </div>
                );
              })}
            </div>
        ...
        <div className="bg-white rounded-2xl shadow-xl p-6 md:p-8">
          ...
          {step === 'body-area' && !loading && (
            <div>
              <h2 className="text-2xl font-bold text-gray-900 mb-2">Where are you experiencing symptoms?</h2>
              <p className="text-gray-600 mb-6">Select the area of your body that&apos;s affected</p>
          ...
          {step === 'symptom' && !loading && (
            <div>
              <button onClick={() => setStep('body-area')} className="text-blue-600 mb-4 flex items-center hover:text-blue-700">
                ← Back
              </button>
              <h2 className="text-2xl font-bold text-gray-900 mb-2">What symptom are you experiencing?</h2>
              <p className="text-gray-600 mb-6">In <strong>{selectedBodyArea?.name}</strong></p>
              ...
              <div className="space-y-3">
                {symptoms.map(symptom => (
                  ...
                  >
                    <h3 className="font-semibold text-lg text-gray-900">{symptom.name}</h3>
          ...
          {step === 'questions' && !loading && (
            <div>
              <button onClick={() => setStep('symptom')} className="text-blue-600 mb-4 flex items-center hover:text-blue-700">
                ← Back
              </button>
              <h2 className="text-2xl font-bold text-gray-900 mb-2">Tell us more about your symptoms</h2>
                    ...
                    {q.question_type === 'single_choice' && (
                      <div className="space-y-2">
                        {q.options.map(option => (
                          <label key={option} className="flex items-center p-3 bg-white rounded-lg cursor-pointer hover:bg-blue-50 border-2 border-transparent hover:border-blue-300">
                            <input
                              type="radio"
                              name={q.id}
                              value={option}
          ...
          {step === 'health-profile' && !loading && (
            <div>
              <button onClick={() => setStep('questions')} className="text-blue-600 mb-4 flex items-center hover:text-blue-700">
                ← Back
              </button>
              <h2 className="text-2xl font-bold text-gray-900 mb-2">Health Profile (Optional)</h2>
              ...
              <div className="space-y-4">
                ...
                <div>
                  <label className="block font-medium text-gray-700 mb-2">Sex Assigned at Birth</label>
                  <select
                    value={healthProfile.sex_at_birth}
                    onChange={(e) => setHealthProfile({...healthProfile, sex_at_birth: e.target.value})}
                    className="w-full p-3 border-2 border-gray-200 rounded-lg focus:border-blue-500 outline-none"
                  >
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setHealthProfile({...healthProfile, chronic_conditions: e.target.value})}
                    className="w-full p-3 border-2 border-gray-200 rounded-lg focus:border-blue-500 outline-none"
          ...
          {step === 'results' && result && (
            <div>
              <div className={`p-6 rounded-xl mb-6 text-white font-bold text-xl
                ${result.triageLevel === 'emergency' ? 'bg-red-600' :
                  result.triageLevel === 'urgent_care' ? 'bg-orange-500' :
                  result.triageLevel === 'pcp' ? 'bg-yellow-500 text-gray-900' :
                  'bg-green-600'}`}>
                Recommendation: {result.triageLevel.replace(/_/g, ' ').toUpperCase()}
              </div>
              ...
              <div className="space-y-6">
                ...
                {result.specialtySuggestion && (
                  <div className="p-4 bg-purple-50 rounded-lg border border-purple-200">
                    <h3 className="font-semibold text-purple-900 mb-1">Suggested Specialty</h3>
                    <p className="text-purple-800">{result.specialtySuggestion}</p>
                  </div>
                )}
                ...
                <div className="p-4 bg-blue-50 rounded-lg border border-blue-200">
                  <h3 className="font-semibold text-blue-900 mb-2">Home Care Advice</h3>
                  <p className="text-blue-800">{result.homeCareAdvice}</p>
                </div>
                
                <div className="p-4 bg-green-50 rounded-lg border border-green-200">
                  <h3 className="font-semibold text-green-900 mb-2">Doctor Visit Preparation</h3>
                  <p className="text-green-800">{result.doctorVisitPreparationTips}</p>
                </div>
              </div>
        ...
        <p className="text-center text-sm text-gray-500 mt-6 max-w-2xl mx-auto">
          ⚠️ <strong>Medical Disclaimer:</strong> This tool provides informational guidance only and is not a substitute for professional medical advice, diagnosis, or treatment. Always consult a qualified healthcare provider for medical concerns.
        </p>
      </div>
    </main>
  );
